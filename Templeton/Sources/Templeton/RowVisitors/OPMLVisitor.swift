...
class OPMLVisitor {
	...
	func visitor(_ visited: Row) {
		...
		let escapedText = visited.topicMarkdown(representation: .opml)?.escapingSpecialXMLCharacters ?? ""
		
		opml.append(indent + "<outline text=\"\(escapedText)\"")
		if let escapedNote = visited.noteMarkdown(representation: .opml)?.escapingSpecialXMLCharacters {
